import { API_URL, getToken, getEmail, requireLogin, fetchCSRFToken } from '../lib/api';
    ...
    async function deriveMessageKey(sharedSecret: ArrayBuffer, info: string, saltBuffer: ArrayBuffer): Promise<CryptoKey> {
      ...
      const infoWithPepper = info + e2eePepper;
      ...
      return crypto.subtle.deriveKey(
        {
          ...
          info: encoder.encode(infoWithPepper)
        },
        baseKey,
    ...
    async function encryptMessage(plaintext: string, key: CryptoKey): Promise<{ ciphertext: string, nonce: string }> {
      const encoder = new TextEncoder();
    ...
    async function decryptMessage(ciphertextB64: string, nonceB64: string, key: CryptoKey): Promise<string> {
    ...
    async function getReceiverPublicKey(receiverEmail: string): Promise<string | null> {
      ...
      const messageKey = await deriveMessageKey(sharedSecret, 'message-encryption', saltHash);
